...
// make service select box a combo box	
	...
	m.attachEvent("onChange",function(id,index,value){   // form client name is changed
		var inputName = $(this).attr('name')  ;
	...
	}) ; // end of : form client phone is changed	
	...
	z.attachEvent("onChange",function(id,index,value){ // form service name is changed
		...
		$("div#hidden-div-services").load( frontpage + "joury/register/service/services/result/salon" ,{"servicename":servicename} , function(){
			// service table is loaded
					
			// staff ajax auto complete
			$("input#staff").autocomplete({source: frontpage + "joury/register/service/autocomplete/staffname"});							
